...
# Version 1.6 - Full English UI Translation
...
# --- CONSTANTS AND DATA (TRANSLATED) ---
GDPR_TERMS = """
This tool is designed to process and filter data from spreadsheets.
It is possible that the files you upload contain sensitive personal data
(such as full name, date of birth, national ID, health information, etc.), the processing of which is regulated by data protection laws like GDPR.

It is your sole responsibility to ensure that all data used in this tool complies with applicable data protection regulations. We strongly recommend that you only use previously
anonymized data to protect the privacy of data subjects.

The responsibility for the nature of the processed data is exclusively yours.

To proceed, you must confirm that the data to be used has been duly treated and anonymized.
"""
...
Navigate through the topics in the left menu to learn how to use each part of the tool.""",
    "1. Global Settings": """**1. Global Settings**

This section, located at the top of the window, contains the essential settings shared between both tools. Configure them once to use in both tabs.

- **Select Spreadsheet:**
  Upload your source data file. Supports `.xlsx`, `.xls`, and `.csv` formats. Once selected, the file is available for both tools.

- **Age Column / Sex/Gender Column:**
  Fields to **select** the exact header name of the column in your spreadsheet. The options in the list appear after the file is uploaded.

- **Sex/Gender (Values) for Stratification:**
  After selecting the "Sex/Gender" column, this section will be populated with the unique values from that column. Select the values you wish to use for stratification.
...
- **Operator and Value:** These define the rule's logic.
...
- **Compound Logic:** Expands the rule to create `AND` / `OR` conditions for creating exclusion ranges.

- **Condition:** Allows applying a secondary filter. The main rule will only be applied to rows that also meet the specified age and/or sex conditions.

- **Actions:** The `X` button deletes a rule. The 'Clone' button duplicates it.

- **Generate Filtered Sheet:** Starts the process. A download button will appear at the end.""",
...
- **Stratify by Sex/Gender Options:**
...
How to use:
ENTRE: Excludes values within the interval (inclusive). Ex: BETWEEN 10 and 20 removes everything from 10 to 20.
OU: Excludes values outside an interval. Use to keep the data in between. Ex: < 10 OR > 20 removes everything less than 10 and greater than 20.
E: Excludes values within an interval, without the extremes. Ex: > 10 AND < 20 removes from 11 to 19 (keeps the values 10 and 20).
"""
    ...
    for i, rule in enumerate(st.session_state.filter_rules):
        with st.container():
            ...
            ops = ["", ">", "<", "=", "Não é igual a", "≥", "≤"]
            rule['p_op1'] = cols[2].selectbox("Operator 1", ops, index=ops.index(rule['p_op1']) if rule.get('p_op1') in ops else 0, key=f"p_op1_{rule['id']}", label_visibility="collapsed")
            ...
            with cols[5]:
                if rule['p_expand']:
                    ...
                    rule['p_op_central'] = exp_cols[0].selectbox("Logic", ops_central, index=ops_central.index(rule['p_op_central']) if rule.get('p_op_central') in ops_central else 0, key=f"p_op_central_{rule['id']}", label_visibility="collapsed")
                    rule['p_op2'] = exp_cols[1].selectbox("Operator 2", ops, index=ops.index(rule.get('p_op2', '>')) if rule.get('p_op2') in ops else 0, key=f"p_op2_{rule['id']}", label_visibility="collapsed")
...
def draw_stratum_rules():
    ...
    for i, stratum_rule in enumerate(st.session_state.stratum_rules):
        with st.container():
            ...
            ops = ["", ">", "<", "≥", "≤"]
            stratum_rule['op1'] = cols[1].selectbox("Operator 1", ops, index=ops.index(stratum_rule['op1']) if stratum_rule['op1'] in ops else 0, key=f"s_op1_{stratum_rule['id']}", label_visibility="collapsed")
            stratum_rule['val1'] = cols[2].text_input("Value 1", value=stratum_rule['val1'], key=f"s_val1_{stratum_rule['id']}", label_visibility="collapsed")
            cols[3].markdown("<p style='text-align: center; margin-top: 25px;'>AND</p>", unsafe_allow_html=True)
            stratum_rule['op2'] = cols[4].selectbox("Operator 2", ops, index=ops.index(stratum_rule['op2']) if stratum_rule['op2'] in ops else 0, key=f"s_op2_{stratum_rule['id']}", label_visibility="collapsed")
            stratum_rule['val2'] = cols[5].text_input("Value 2", value=stratum_rule['val2'], key=f"s_val2_{stratum_rule['id']}", label_visibility="collapsed")
