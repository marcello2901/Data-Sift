...
# Versão 1.7 - Correção de layout e revisão de traduções na interface
...
DEFAULT_FILTERS = [
    ...
    {'id': str(uuid.uuid4()), 'p_check': True, 'p_col': 'Hemo.OBSSV', 'p_op1': 'Not equal to', 'p_val1': 'vazio', 'p_expand': False, 'p_op_central': 'OR', 'p_op2': '<', 'p_val2': '', 'c_check': False, 'c_idade_check': False, 'c_idade_op1': '>', 'c_idade_val1': '', 'c_idade_op2': '<', 'c_idade_val2': '', 'c_sexo_check': False, 'c_sexo_val': ''},
    {'id': str(uuid.uuid4()), 'p_check': True, 'p_col': 'Hemo.OBSSB', 'p_op1': 'Not equal to', 'p_val1': 'vazio', 'p_expand': False, 'p_op_central': 'OR', 'p_op2': '<', 'p_val2': '', 'c_check': False, 'c_idade_check': False, 'c_idade_op1': '>', 'c_idade_val1': '', 'c_idade_op2': '<', 'c_idade_val2': '', 'c_sexo_check': False, 'c_sexo_val': ''},
    {'id': str(uuid.uuid4()), 'p_check': True, 'p_col': 'Hemo.OBSSP', 'p_op1': 'Not equal to', 'p_val1': 'vazio', 'p_expand': False, 'p_op_central': 'OR', 'p_op2': '<', 'p_val2': '', 'c_check': False, 'c_idade_check': False, 'c_idade_op1': '>', 'c_idade_val1': '', 'c_idade_op2': '<', 'c_idade_val2': '', 'c_sexo_check': False, 'c_sexo_val': ''},
]
...
def load_dataframe(uploaded_file):
    ...
    try:
        if uploaded_file.name.endswith('.csv'):
            try:
                uploaded_file.seek(0); return pd.read_csv(uploaded_file, sep=';', decimal=',', encoding='latin-1')
            except Exception:
                uploaded_file.seek(0); return pd.read_csv(uploaded_file, sep=',', decimal='.', encoding='utf-8')
        else:
                ...
                with st.container():
                    ...
                    with cond_cols[3]:
                        if rule['c_idade_check']:
                            age_cols = st.columns([1,1,0.4,1,1])
...
def main():
    ...
    if 'filter_rules' not in st.session_state: 
        ...
        for rule in default_filters_translated:
            if rule.get('p_op1') == 'Não é igual a': rule['p_op1'] = 'Not equal to'
            if rule.get('p_op_central') == 'OU': rule['p_op_central'] = 'OR'
            elif rule.get('p_op_central') == 'E': rule['p_op_central'] = 'AND'
            elif rule.get('p_op_central') == 'ENTRE': rule['p_op_central'] = 'BETWEEN'
                ...
                if df is None: st.error("Please upload a spreadsheet first.")
                    ...
                    with st.spinner("Generating strata... Please wait."):
                        ...
                        for gender_val, is_selected in st.session_state.get('strat_gender_selection', {}).items():
                            if is_selected: sex_rules.append({'value': gender_val, 'name': str(gender_val)})
